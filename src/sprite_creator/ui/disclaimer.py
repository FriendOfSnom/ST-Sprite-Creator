"""
Disclaimer/Terms Screen for ST Sprite Creator.

Shows a disclaimer on every launch requiring user acknowledgment that they
accept responsibility for AI-generated content and that all characters are 18+.
"""
...
DISCLAIMER_TEXT = """Welcome to ST Sprite Creator

This tool uses Google's Gemini AI to generate character sprites for the Student Transfer visual novel.
...
2. All Characters Are 18+
   All characters you create with this tool are adults aged 18 or older, regardless of their appearance or archetype. By proceeding, you acknowledge that every character you generate represents an adult. You agree not to use this tool to create content intended to depict minors.
...
4. Appropriate Use
   This tool is designed for creating character sprites for Student Transfer. You agree to use it responsibly and in compliance with all applicable laws.
...
This disclaimer is shown every time the app is launched."""
...
_disclaimer_accepted_this_session = False


def show_disclaimer_if_needed() -> bool:
    """
    Show the disclaimer window on every app launch.

    Only shown once per process — returning to the launcher after a tool
    finishes won't re-prompt.

    Returns:
        True if user has accepted, False if declined
    """
    global _disclaimer_accepted_this_session
    if _disclaimer_accepted_this_session:
        return True
    ...
    if accepted:
        _disclaimer_accepted_this_session = True
